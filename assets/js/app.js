(function(){
  ...
  const COMPAT_RULES = {
    2:{label:'2ND GEN',ram:'DDR3',chipsets:['H61','B75','H77'],socket:'LGA1155'},
    3:{label:'3RD GEN',ram:'DDR3',chipsets:['H61','B75','H77'],socket:'LGA1155'},
    4:{label:'4TH GEN',ram:'DDR3',chipsets:['H81','B85','H87','Z87','H97','Z97'],socket:'LGA1150'},
    5:{label:'5TH GEN',ram:'DDR3',chipsets:['H81','B85','H97','Z97'],socket:'LGA1150'},
    6:{label:'6TH GEN',ram:'DDR4',chipsets:['H110','B150','H170','Z170'],socket:'LGA1151'},
    7:{label:'7TH GEN',ram:'DDR4',chipsets:['H110','B250','H270','Z270'],socket:'LGA1151'},
    8:{label:'8TH GEN',ram:'DDR4',chipsets:['H310','B360','B365','H370','Z370'],socket:'LGA1151'},
    9:{label:'9TH GEN',ram:'DDR4',chipsets:['H310','B360','B365','H370','Z390'],socket:'LGA1151'},
    10:{label:'10TH GEN',ram:'DDR4',chipsets:['H410','B460','H470','Z490'],socket:'LGA1200'}
  };
  ...
  function genNum(v){const s=upper(v);if(!s)return '';const m=s.match(/(10|[2-9])/);return m?Number(m[1]):''}
  function genLabel(g){g=Number(g);if(!g)return '';return (g===2?'2ND':g===3?'3RD':`${g}TH`)+' GEN'}
  ...
  function normalizeProduct(p,i=0){
    const g=genNum(p.generation||p.gen);
    return {
  ...
  function productCard(p){
    return `<article class="card product-card">
      <div class="product-img-wrap"><img src="${esc(productImage(p))}" alt="${esc(p.name)}" onerror="this.src='assets/img/logo.jpg'">
        <div class="tag-row"><span class="tag ${p.condition==='NEW'?'new':'used'}">${esc(p.condition)}</span>${p.memory?`<span class="tag">${esc(p.memory)}</span>`:''}${p.generation?`<span class="tag">${esc(p.generationLabel)}</span>`:''}</div>
      </div>
      <div class="product-body"><h3>${esc(p.name)}</h3>
        <div class="meta"><span>${esc(p.category)}</span>${p.chipset?`<span>${esc(p.chipset)}</span>`:''}${p.socket?`<span>${esc(p.socket)}</span>`:''}</div>
        <div class="price">${money(p.price)}</div><div class="stock">Stock: ${esc(p.stock||'Contact')}</div>
        <div class="product-actions"><button class="btn btn-primary btn-small" data-add-to-cart="${esc(p.id)}">Add to cart</button><a class="btn btn-ghost btn-small" href="checkout.html">Checkout</a></div>
      </div></article>`;
  }
  ...
  async function initProductsPage(){
    ...
    const inputs=['#productSearch','#categoryFilter','#conditionFilter','#memoryFilter','#generationFilter'].map(id=>$(id)).filter(Boolean);
    inputs.forEach(el=>['input','change'].forEach(ev=>el.addEventListener(ev,render)));
    function render(){
      const q=upper($('#productSearch')?.value);const cat=upper($('#categoryFilter')?.value);const cond=upper($('#conditionFilter')?.value);const mem=upper($('#memoryFilter')?.value);const gen=Number($('#generationFilter')?.value||0);
      const filtered=products.filter(p=>{
        const hay=upper([p.name,p.category,p.condition,p.memory,p.generationLabel,p.chipset,p.socket,p.description].join(' '));
        return (!q||hay.includes(q))&&(!cat||p.category===cat)&&(!cond||p.condition===cond)&&(!mem||p.memory===mem)&&(!gen||Number(p.generation)===gen);
      });
  ...
  function prebuildCard(p){
    return `<article class="card prebuild-card">
      ...
      <div class="prebuild-body"><h3>${esc(p.title)}</h3><div class="price">${esc(p.price)}</div>
        ...
        <div class="product-actions"><button class="btn btn-whatsapp btn-small" data-prebuild-whatsapp="${esc(p.id)}">Order on WhatsApp</button><a class="btn btn-ghost btn-small" href="quotation.html">Customize</a></div>
      </div>
    </article>`;
  }
  function initPrebuildPage(){const grid=$('#prebuildGrid');if(grid)grid.innerHTML=PREBUILDS.map(prebuildCard).join('')}
...
Please confirm availability and final price.`;
  ...
  async function initQuotationPage(){
    ...
    const processor=$('#quoteProcessor'),mobo=$('#quoteMotherboard'),ram=$('#quoteRam');if(!processor)return;
    ...
    processor.addEventListener('change',()=>{
      const cpu=byId(products,processor.value);const rule=cpu?COMPAT_RULES[cpu.generation]:null;
      if(!rule){mobo.innerHTML='<option value="">Select processor first</option>';ram.innerHTML='<option value="">Select processor first</option>';mobo.disabled=true;ram.disabled=true;$('#compatNote').textContent='Processor එක select කළාම compatible motherboard සහ RAM පෙන්වයි.';renderQuote();return;}
      const motherboards=products.filter(p=>p.category==='MOTHERBOARD'&&p.memory===rule.ram&&(rule.chipsets.includes(p.chipset)||p.socket===rule.socket||Number(p.generation)===Number(cpu.generation)));
      ...
      $('#compatNote').innerHTML=`<b>${esc(cpu.name)}</b> selected. Compatible: <b>${esc(rule.chipsets.join(' / '))}</b> motherboards • <b>${esc(rule.ram)}</b> RAM • <b>${esc(rule.socket)}</b>.`;
      renderQuote();
    });
    [mobo,ram].forEach(el=>el?.addEventListener('change',renderQuote));
